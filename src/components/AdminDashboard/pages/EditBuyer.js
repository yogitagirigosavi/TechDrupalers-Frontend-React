...
const  EditBuyer = () => 
{
    ...
    const buyer = JSON.parse(localStorage.getItem("Buyer Data"));
    const buyerUUID = buyer[0].uuid;
    //console.log(buyerUUID);

    const[email, emailchange] = useState(buyer[0].mail || "");
    const[username, usernamechange] = useState(buyer[0].name || "");
    const[password, passwordchange] = useState(buyer[0].pass || "");
    const[confpassword, confpasswordchange] = useState(buyer[0].pass || "");
    const[compname, compnamechange] = useState(buyer[0].field_company_name || "");
    const[countries, countrieschange] = useState(buyer[0].field_address_country_code || "");
    const[streetadd, streeetaddchange] = useState(buyer[0].field_address_address_line1 || "");
    const[city,citychange] = useState(buyer[0].field_address_locality || "");
    const[pincode,pincodechange] = useState(buyer[0].field_address_postal_code || "");
    const[state,statechange] = useState(buyer[0].field_address_administrative_area || "");
    ...
    const handlesubmit = (e) => 
    {
        ...
        //console.log(regobj);
        updateBuyer(buyerUUID);
    }

    const updateBuyer = (uuidbuyer) =>
    {
        const requestBody = 
        {
            "data": 
        ...
        // Define the API URL
        const apiUrl = `http://localhost/TechDrupalers/web/jsonapi/user/user/${uuidbuyer}`;
    ...
    return (
        <>
            ...
            <div className="edit-buyer-pagetitle">
                <h2>Edit User</h2>
            </div>
